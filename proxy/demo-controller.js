/**
 * ╔═══════════════════════════════════════════════════════════════╗
 * ║   ORCHATHON — Live Hackathon Demo Controller                 ║
 * ║   Serves the presentation UI on port 3000                   ║
 * ╚═══════════════════════════════════════════════════════════════╝
 *
 * This is the PRESENTATION TOOL for judges.
 * Open http://localhost:3000 on a projector.
 *
 * Shows 3 panels side by side:
 *   LEFT  — Target website (ShopSecure)
 *   CENTER — Attack console (running live attacks)
 *   RIGHT — Proxy defense dashboard
 *
 * Buttons trigger real HTTP attacks against the proxy/backend.
 */
...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .mode-tab {
    flex:1; padding:10px 0; font-family:var(--mono); font-size:11px; font-weight:700;
  ...
  .mode-tab.active-unprotected { color:var(--red); border-bottom-color:var(--red); background:rgba(255,51,85,.06); }
  .mode-tab.active-protected   { color:var(--green); border-bottom-color:var(--green); background:rgba(0,255,136,.05); }
  .mode-tab:hover:not(.active-unprotected):not(.active-protected) { color:var(--text); background:rgba(255,255,255,.03); }

  /* ── 3-panel layout ── */
  .view { display:none; height:calc(100vh - 54px - 42px); flex-direction:column; }
  .view.active { display:flex; }
  .panels { display:grid; grid-template-columns:1fr 340px 1fr; flex:1; gap:1px; background:var(--border); overflow:hidden; }
...
<!-- Mode tabs -->
<div class="mode-tabs">
  <button class="mode-tab active-unprotected" id="tab-unprotected" onclick="switchMode(false)">
    ⚠ WITHOUT PROXY &nbsp;<span style="font-size:9px;opacity:.7">→ :8080 direct — no blocking</span>
  </button>
  <button class="mode-tab" id="tab-protected" onclick="switchMode(true)">
    🛡 WITH PROXY &nbsp;<span style="font-size:9px;opacity:.7">→ :9090 shielded — attacks blocked</span>
  </button>
</div>

<!-- ════════════════════════════════════════════ -->
<!-- VIEW A: WITHOUT PROXY (port 8080)           -->
<!-- ════════════════════════════════════════════ -->
<div class="view active" id="view-unprotected">
  <div class="panels">

    <!-- LEFT: Website WITHOUT protection -->
    ...
    <div class="panel console-panel">
      <div class="panel-header">
        <div class="panel-title red">② Attacker Console</div>
        <div class="panel-status" id="a-atk-status" style="color:var(--muted)">ready · hitting :8080</div>
      </div>
      <div class="attack-btns">
        <button class="atk-btn ddos"      onclick="attack('ddos','a')">      <span class="icon">💥</span>DDoS Flood</button>
        <button class="atk-btn brute"     onclick="attack('bruteforce','a')"> <span class="icon">🔓</span>Brute Force</button>
        <button class="atk-btn sql"       onclick="attack('sqlinject','a')">  <span class="icon">💉</span>SQL Inject</button>
        <button class="atk-btn xss"       onclick="attack('xss','a')">        <span class="icon">🕸</span>XSS Attack</button>
        <button class="atk-btn blacklist" onclick="attack('blacklist','a')">  <span class="icon">🚫</span>Blacklist</button>
        <button class="atk-btn legit"     onclick="attack('legit','a')">      <span class="icon">✅</span>Legit User</button>
      </div>
      <div class="console-stats">
        <div class="cs"><div class="cs-val" id="a-cs-total" style="color:var(--accent)">0</div><div class="cs-label">SENT</div></div>
        <div class="cs"><div class="cs-val" id="a-cs-blocked" style="color:var(--red)">0</div><div class="cs-label">BLOCKED</div></div>
        <div class="cs"><div class="cs-val" id="a-cs-passed" style="color:var(--green)">0</div><div class="cs-label">PASSED</div></div>
      </div>
      <div class="console-log" id="a-console-log">
        <div style="color:var(--muted);font-size:11px;text-align:center;padding:32px 0">
          <span style="color:var(--red)">⚠ No proxy active</span> — all attacks reach backend directly.<br><br>
          Run attacks to see how unprotected the site is.
        </div>
      </div>
    </div>

    <!-- RIGHT: Proxy Shield Dashboard (still shows stats from proxy for reference) -->
    <div class="panel">
      <div class="panel-header">
        ...
        <button onclick="const f=document.getElementById('a-dash-frame');f.src='';setTimeout(()=>f.src='http://localhost:9091',50)" style="margin-left:8px;padding:3px 10px;border-radius:4px;font-family:var(--mono);font-size:10px;cursor:pointer;background:rgba(0,255,136,.08);color:var(--green);border:1px solid rgba(0,255,136,.3);">↺ RELOAD</button>
      </div>
      <iframe class="dash-frame" id="a-dash-frame" src="http://localhost:9091"></iframe>
    </div>
...
<!-- ════════════════════════════════════════════ -->
<!-- VIEW B: WITH PROXY (port 9090)             -->
<!-- ════════════════════════════════════════════ -->
<div class="view" id="view-protected">
  <div class="panels">

    <!-- LEFT: Website WITH protection -->
    ...
    <div class="panel console-panel">
      <div class="panel-header">
        <div class="panel-title red">② Attacker Console</div>
        <div class="panel-status" id="b-atk-status" style="color:var(--muted)">ready · hitting :9090</div>
      </div>
      <div class="attack-btns">
        <button class="atk-btn ddos"      onclick="attack('ddos','b')">      <span class="icon">💥</span>DDoS Flood</button>
        <button class="atk-btn brute"     onclick="attack('bruteforce','b')"> <span class="icon">🔓</span>Brute Force</button>
        <button class="atk-btn sql"       onclick="attack('sqlinject','b')">  <span class="icon">💉</span>SQL Inject</button>
        <button class="atk-btn xss"       onclick="attack('xss','b')">        <span class="icon">🕸</span>XSS Attack</button>
        <button class="atk-btn blacklist" onclick="attack('blacklist','b')">  <span class="icon">🚫</span>Blacklist</button>
        <button class="atk-btn legit"     onclick="attack('legit','b')">      <span class="icon">✅</span>Legit User</button>
      </div>
      <div class="console-stats">
        <div class="cs"><div class="cs-val" id="b-cs-total" style="color:var(--accent)">0</div><div class="cs-label">SENT</div></div>
        <div class="cs"><div class="cs-val" id="b-cs-blocked" style="color:var(--red)">0</div><div class="cs-label">BLOCKED</div></div>
        <div class="cs"><div class="cs-val" id="b-cs-passed" style="color:var(--green)">0</div><div class="cs-label">PASSED</div></div>
      </div>
      <div class="console-log" id="b-console-log">
        <div style="color:var(--muted);font-size:11px;text-align:center;padding:32px 0">
          <span style="color:var(--green)">🛡 OrchProxy active</span> — attacks will be blocked.<br><br>
          Run the same attacks to see the proxy shield in action.
        </div>
      </div>
    </div>

    <!-- RIGHT: Proxy Shield Dashboard -->
    <div class="panel">
      <div class="panel-header">
        <div class="panel-title green">③ Proxy Shield Dashboard</div>
        <div class="panel-status" style="color:var(--green)">http://localhost:9091 — LIVE</div>
        <button onclick="const f=document.getElementById('b-dash-frame');f.src='';setTimeout(()=>f.src='http://localhost:9091',50)" style="margin-left:8px;padding:3px 10px;border-radius:4px;font-family:var(--mono);font-size:10px;cursor:pointer;background:rgba(0,255,136,.08);color:var(--green);border:1px solid rgba(0,255,136,.3);">↺ RELOAD</button>
      </div>
...
// Per-view state
const state = {
  a: { totals: { sent:0, blocked:0, passed:0 }, running:false },
  b: { totals: { sent:0, blocked:0, passed:0 }, running:false },
};

let activeView = 'a'; // 'a' = unprotected, 'b' = protected

function switchMode(toProxy) {
  activeView = toProxy ? 'b' : 'a';

  document.getElementById('view-unprotected').classList.toggle('active', !toProxy);
  document.getElementById('view-protected').classList.toggle('active', toProxy);

  const tabU = document.getElementById('tab-unprotected');
  const tabP = document.getElementById('tab-protected');
  tabU.className = 'mode-tab' + (!toProxy ? ' active-unprotected' : '');
  tabP.className = 'mode-tab' + (toProxy  ? ' active-protected'   : '');

  document.getElementById('mode-badge').textContent  = toProxy ? '🛡 PROTECTED' : '⚠ UNPROTECTED';
  document.getElementById('mode-badge').className    = 'mode-badge ' + (toProxy ? 'mode-protected' : 'mode-unprotected');
}

function updateCounters(v) {
  const t = state[v].totals;
  document.getElementById(v+'-cs-total').textContent   = t.sent;
  document.getElementById(v+'-cs-blocked').textContent = t.blocked;
  document.getElementById(v+'-cs-passed').textContent  = t.passed;
}

function addSysLog(v, msg, type) {
  const log = document.getElementById(v+'-console-log');
...
function showGlitch(v, on, label='') {
  const overlay = document.getElementById(v+'-atk-overlay');
  const text    = document.getElementById(v+'-atk-text');
  overlay.classList.toggle('active', on);
  if (on) { text.textContent = '⚠ '+label; text.classList.add('visible'); }
  else    { text.classList.remove('visible'); }
}

async function attack(type, v) {
  if (state[v].running) return;
  state[v].running = true;
  ...
  document.querySelectorAll('#view-'+(useProxy?'protected':'unprotected')+' .atk-btn')
    .forEach(b => b.classList.add('running'));
  ...
  try {
    ...
    if (isHostile) showGlitch(v, false);
    ...
    results.forEach(entry => {
      ...
      if (blocked) { state[v].totals.blocked++; attackBlocked++; }
      else         { state[v].totals.passed++;  attackPassed++;  }
    ...
    addSysLog(v, summary, useProxy && isHostile ? 'success' : isHostile ? 'danger' : 'success');
    document.getElementById(v+'-atk-status').textContent = \`done — \${results.length} requests\`;

  } catch(e) {
    showGlitch(v, false);
    addSysLog(v, 'Error: '+e.message, 'danger');
  }

  document.querySelectorAll('#view-'+(useProxy?'protected':'unprotected')+' .atk-btn')
    .forEach(b => b.classList.remove('running'));
  state[v].running = false;
}
</script>
</body>
</html>`;
...
const server = http.createServer(async (req, res) => {
  ...
  if (parsed.pathname === '/run-attack' && req.method === 'POST') {
    ...
    req.on('end', async () => {
      const { type: requestType, useProxy } = JSON.parse(body);
      ...
      const results = await runAttackOn(requestType, targetPort);
...
async function runAttackOn(type, targetPort) {
  ...
  // Port-aware request builder — shadows the global makeReq for this scope
  const makeReq = (opts) => makeReqOn(targetPort, opts);
  ...
  // ════════════════════════════════════════════
    ...
    const classify = (status, responseTime) => {
      if (status === 400 || status === 403) return { icon: '🛡', verdict: 'BLOCKED by WAF',            critical: false, wafBlocked: true  };
      if (status === 500)                   return { icon: '💥', verdict: 'SERVER ERROR — likely leak', critical: true,  wafBlocked: false };
      if (status === 200 && responseTime > 4000) return { icon: '⏱', verdict: 'TIME-BASED BLIND HIT',  critical: true,  wafBlocked: false };
      if (status === 200)                   return { icon: '⚠️', verdict: 'REACHED BACKEND!',           critical: true,  wafBlocked: false };
      if (status === 401 || status === 404) return { icon: '🔒', verdict: 'Auth/Not found — low risk',  critical: false, wafBlocked: false };
      return                                       { icon: '❓', verdict: `Unexpected: ${status}`,      critical: false, wafBlocked: false };
    };
    ...
    for (const batch of chunk(ALL_PAYLOADS, CONFIG.concurrency)) {
      await Promise.allSettled(
        batch.map(async ({ path, label, method, body, category }) => {
          ...
          const responseTime = Date.now() - t0;
          const { icon, verdict, wafBlocked } = classify(r.status, responseTime);

          add(icon,
            `[${category.toUpperCase()}] #${attemptNum} — ${label}`,
            `${r.status} (${responseTime}ms)`,
            verdict,
            wafBlocked   // true ONLY when proxy returned 400/403
          );
        })
      );
    }
  ...
  // ════════════════════════════════════════════
    ...
    const classify = (status, responseText = '') => {
      ...
      if (status === 400 || status === 403) return { icon: '🛡', verdict: 'BLOCKED by WAF',                   wafBlocked: true  };
      if (echoed)                           return { icon: '🚨', verdict: 'REFLECTED — payload echoed back!', wafBlocked: false };
      if (status === 500)                   return { icon: '💥', verdict: 'SERVER ERROR',                     wafBlocked: false };
      if (status === 200)                   return { icon: '⚠️', verdict: 'REACHED BACKEND — inspect!',       wafBlocked: false };
      if (status === 301 || status === 302) return { icon: '↪️', verdict: 'Redirect — check Location header', wafBlocked: false };
      if (status === 404 || status === 401) return { icon: '🔒', verdict: 'Auth/Not found — low risk',        wafBlocked: false };
      return                                       { icon: '❓', verdict: `Unexpected: ${status}`,            wafBlocked: false };
    };
    ...
    for (const batch of chunk(ALL_PAYLOADS, CONFIG.concurrency)) {
      await Promise.allSettled(
        batch.map(async ({ path, label, method, body, category }) => {
          ...
          const { icon, verdict, wafBlocked } = classify(r.status, responseText);
  ...
  // ════════════════════════════════════════════
    ...
    const classify = (status, category) => {
      ...
      if (status === 403 || status === 429)
        return expectBlock
          ? { icon: '⛔', verdict: 'BLOCKED — correct',              wafBlocked: true  }
          : { icon: '⚠️', verdict: 'FALSE POSITIVE — clean blocked', wafBlocked: false };
      if (status === 200 || status === 204)
        return expectBlock
          ? { icon: '🚨', verdict: 'BYPASS — blacklisted IP passed!', wafBlocked: false }
          : { icon: '✅', verdict: 'ALLOWED — correct',               wafBlocked: false };
      if (status === 401 || status === 404)
        return expectBlock
          ? { icon: '⚠️', verdict: 'Not explicitly blocked',         wafBlocked: false }
          : { icon: '✅', verdict: 'Reached app layer — correct',     wafBlocked: false };
      return { icon: '❓', verdict: `Unexpected: ${status}`, wafBlocked: false };
    };
    ...
    for (const batch of chunk(ALL_ATTEMPTS, CONFIG.concurrency)) {
      await Promise.allSettled(
        batch.map(async ({ category, ip, label, endpoint, headerKey, chainWith }) => {
          ...
          const { icon, verdict, wafBlocked } = classify(r.status, category);

          add(icon,
            `[${category.toUpperCase()}] #${attemptNum} — ${label} → ${endpoint}`,
            r.status,
            verdict,
            wafBlocked   // true ONLY when proxy returned 403/429 for a known bad IP
          );
        })
      );
    }
...
server.listen(DEMO_PORT, () => {
  console.log('');
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   ORCHATHON — Hackathon Demo Controller                 ║');
  console.log('╠══════════════════════════════════════════════════════════╣');
  console.log('║                                                          ║');
  console.log('║   🎯 OPEN THIS ON THE PROJECTOR:                        ║');
  console.log('║      http://localhost:3000                              ║');
  console.log('║                                                          ║');
  console.log('║   Port Map:                                              ║');
  console.log('║   :3000 → This demo controller (presentation)           ║');
  console.log('║   :8080 → ShopSecure backend (target website)           ║');
  console.log('║   :9090 → OrchProxy (the invisible middleman)           ║');
  console.log('║   :9091 → Live security dashboard                       ║');
  console.log('║                                                          ║');
  console.log('╚══════════════════════════════════════════════════════════╝');
  console.log('');
});